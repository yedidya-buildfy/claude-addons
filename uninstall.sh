...
rm -f "$CLAUDE_DIR/scripts/agent-locks.mjs"
rm -f "$CLAUDE_DIR/scripts/hebrew-guard.py"
rm -rf "$CLAUDE_DIR/agent-locks"
...
if [ -f "$CLAUDE_SETTINGS" ]; then
  node -e '
    ...
    if (cfg.hooks) {
      const isOurs = h => ["tab.sh", "tab-autoname.py", "ntfy.sh", "agent-locks.mjs", "claude-addons-update.sh", "hebrew-guard.py"]
