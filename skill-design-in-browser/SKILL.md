...
1. **Read the project's design system first** (never invent tokens). Look for and
   read whatever exists: a `DESIGN.md` / `PRODUCT.md` / style guide; the CSS theme
...
3. **Open it in the browser** so the user can look:
   ...
   - Re-run after each edit; the user refreshes. (If a dev server with the file is
     already running, just give them the URL instead.)
...
## Notes
- If the **impeccable** design plugin is installed, use its quality vocabulary; snap
  any off-scale radii to the token scale.
...
- A per-project copy of this skill can add project-specific paths/tokens and a
  gold-standard example file to copy.
