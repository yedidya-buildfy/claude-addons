...
| תוסף | מה זה עושה |
|---|---|
| [**tab-status**](./tab-status) | נקודה צבעונית (⚪🔴🟣🟠🔵🟡🟢) על ה‑tab של הטרמינל ב‑VS Code, שמראה אם Claude פנוי, עובד, בונה תוכנית (🟣), מחכה להחלטה שלך על תוכנית (🟠), מחכה לתשובה שלך (🔵), או מחכה לסוכן רקע (🟡). אחרי עבודה שדחפה ל‑`main`/`master` הירוק מתחלף ב‑✅, כדי לדעת אילו לשוניות כבר עלו לפני שסוגרים אותן. |
...
| [**agent-locks**](./agent-locks) | שומר לכמה סשנים שחולקים תיקייה אחת: לפני כתיבה לקובץ שסשן חי אחר מחזיק, ולפני דחיפה או העלאה בזמן שסשן אחר באמצע כזו, מופיעה התראה שאומרת מי מחזיק ומאיזו שעה. רק מזהיר, אף פעם לא חוסם — התראת שווא שעוצרת עבודה גרועה יותר. התפיסה לפי הנתיב המלא, כך שסשן שעובד בעותק עבודה משלו לא מתנגש ולא מוזכר; זו גם הדחיפה לעבוד ככה. משתחרר לבד ברגע שהסשן ממתין למשתמש, ואחרי רבע שעה שקטה אם הסשן קרס. `agent-locks.mjs list` מראה מי מחזיק מה. |
| [**hebrew-guard**](./hebrew-guard) | עברית קריאה בטרמינל של VS Code. בכל פתיחת סשן בודק שני דברים ומתקן לבד: שהטרמינל מצייר עם כרטיס המסך (כבוי = האותיות בתוך המילים יוצאות הפוכות), ושאין תוסף "תיקון עברית" פעיל (VS Code כבר מסדר עברית בעצמו, אז תוסף כזה הופך אותה פעמיים). שקט כשהכל תקין, ומודיע כשתיקן. טקסט דהוי בטרמינל? Reload Window — לא לכבות את כרטיס המסך. מותקן תמיד. |
...
| [**auto-claude**](./auto-claude) | כל טרמינל חדש נפתח ישר בתוך `claude --dangerously-skip-permissions`. `Ctrl-C` פעמיים ואתה בטרמינל רגיל. לא נפתח ב‑SSH, בפקודות שקלוד מריץ או ב‑shell מקונן. ⚠️ בלי בקשות אישור. |
...
ההתקנה מיישמת ברירות מחדל (או, במחשב שכבר היו בו תוספים, בדיוק את מה שהיה מותקן) ופותחת דף הגדרות בדפדפן.

## ניהול: הפקודה `addons`

כל תוסף מתואר פעם אחת, בקובץ `addon.json` בתיקייה שלו, ומנוע אחד מתקין, מעדכן ומסיר את כולם.

| פקודה | מה היא עושה |
|---|---|
| `addons` | דף הגדרות מקומי: מתג לכל תוסף, הסבר מלא, והגדרות |
| `addons status` | מה דלוק, ואם משהו שונה ידנית |
| `addons on <id>` / `addons off <id>` | הדלקה או כיבוי של תוסף אחד |
| `addons apply --dry-run` | מה ישתנה, בלי לשנות כלום |

הבחירות נשמרות במחשב עצמו (`~/.claude/addons/config.json`), לא במאגר. כל שינוי נעשה בבת אחת או בכלל לא, עם גיבוי של כל קובץ שנגע בו.
בקובץ ההפעלה של הטרמינל יש אזור אחד מנוהל; כל מה שמחוץ לו לא נוגעים בו.
...
מסיר בדיוק את מה שהתוספים כתבו, ושום דבר אחר. הנושא הפרטי של התראות הטלפון נשמר, כדי שהתקנה מחדש תמשיך לאותו מינוי.
